...
#### The Zoning Tab
* **1. All Metrics/All Panes** *(default tab)*: The ultimate demo builder. Set Min/Max bounds for *every* metric in the library (Revenue, Click Rate, Attractiveness, Time, etc.). Clicking **🚀 Generate All Data** injects tailored data for every single metric into all visible zones simultaneously. Exposure Rate and Exposure Time realistically stay near their max value for the top portion of the page (since nearly everyone scrolls that far) before decaying further down. In Compare Mode, use **Right Pane Variance** to control how much lower (or, with a negative number, higher) the right pane's values run compared to the left — set it to 0 for a neutral, unbiased comparison.
* **2. Single Metric/Single Pane**: Instantly populate all un-edited zones on the screen with data for the metric you are currently viewing. You can choose to overwrite existing native data (>0) or only fill the blanks. Data is scaled between your chosen Min and Max values. Intended for single-screen use — it does not apply the Right Pane Variance used by All Metrics/All Panes.
* **3. Exposure**: Quickly build a realistic "scroll depth" narrative for Exposure Rate or Exposure Time (units follow whichever is currently selected). Defines a Top and Bottom value — zones above the fold are automatically assigned the Top value, and zones below the fold gradually decrease toward the Bottom value. Supports a fixed or viewport-relative fold position, independent per-pane bounds, and skipping zones that already have other edits applied.
...
* Compare Mode (split-screen) is fully and reliably supported by All Metrics/All Panes — each pane independently and correctly reflects its own selected metric or segment. Single Metric/Single Pane is intended for single-screen use only.