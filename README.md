# Contentsquare Zoning Demo Tool

A Chrome extension designed for PreSales and Solution Consulting teams to easily mock up and modify Contentsquare Zoning analysis data directly in the browser.

## Features

* **Live Editing**: Click on any zone in the Contentsquare Zoning module to instantly change its metric value and display text.
...
1.  Navigate to a Zoning analysis inside the Contentsquare platform.
...
* **2. Global Metric Library Tuner (360° Data Generation)**: The ultimate demo builder. Instead of generating data for just one metric, this allows you to set Min/Max bounds for *every* metric in the library (Revenue, Click Rate, Attractiveness, Time, etc.). Clicking **🚀 Generate All Data** injects tailored data for every single metric into all visible zones simultaneously. You can then seamlessly switch between metrics in the Contentsquare UI dropdown, and the extension will instantly swap the data on screen to match the active tab!
...
* **Reset All**: Clears all mocked data and restores the page to the native Contentsquare values.

## Important Notes

* This extension works by manipulating the DOM locally in your browser. It **does not** send any data to Contentsquare servers or permanently alter the actual analysis.
